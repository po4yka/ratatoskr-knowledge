...
- [ ] 4.7 Wire the poll loop into the service lifecycle per design D5 (immediate first pass, interval sleep, drain-aware stop inside the shutdown bound) in `services/knowledge`. Cannot start from a failing test: composition-root wiring verified by the gate's build, config, and database suites.
