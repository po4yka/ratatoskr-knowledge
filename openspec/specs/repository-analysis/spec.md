...
Knowledge SHALL account for repository provider usage against the shared daily ledger before execution, so the configured global and family limits apply across article and repository runs.

#### Scenario: Exhausted shared budget blocks repository execution

- **WHEN** earlier accepted family work exhausts the applicable daily budget
